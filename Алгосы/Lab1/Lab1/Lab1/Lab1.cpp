...
std::vector<double> solve_2(std::vector<double> coef) {
    if (coef.size() != 3) {
        throw "Функция не является полином 2-й степени";
    }
...
std::vector<double> solve(std::vector<double> coef, double tool, double left, double right) {
    if (coef[0] == 0) {
        ...
        return solve(coef, tool, left, right);
        // return cardano_method(coef, tool, left, right);
    }
    ...
    roots_of_df1 = cardano_method(df1);

    if (roots_of_df1.size() == 0) {
        std::cout << "На заданном отрезке [" << left << "," << right << "] нет точек экстремума. ";
        std::cout << "Расширьте границы отрезка" << std::endl;
        return ans;
    }
...
int main() {
    ...
    for (int i = 0; i < 5; i++) {
        ...
        std::string sign = polynom[i] > 0 ? "+" : "";
        std::cout << sign << polynom[i] << "x^" << 5 - i - 1 << " ";
    }
    std::cout << std::endl;

    std::vector<double> test{ -1, -6, 0, 8, -8 };
    std::vector<double> roots;
    try {
        roots = solve(test, precision, left, right);
    }
    catch (const char* err) {
        std::cout << err << std::endl;
        return -1;
    }
    std::cout << "Корни исходного уравнения:" << std::endl;
    ...
    if (count == 5) {
        ...
        std::cout << "Корни отсутствуют" << std::endl;
    }
    std::cout << std::endl;
